...
from VishnuRobot import DRAGONS
from VishnuRobot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def vishnu_is_admin(chat_id: int):
    status = False
    vishnu = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if vishnu.id == user.id:
            status = True
            break
    return status
