...
import VishnuRobot.modules.sql.userinfo_sql as sql
...
from VishnuRobot.__main__ import STATS, TOKEN, USER_INFO
from VishnuRobot.modules.disable import DisableAbleCommandHandler
from VishnuRobot.modules.helper_funcs.chat_status import sudo_plus
from VishnuRobot.modules.helper_funcs.extraction import extract_user
from VishnuRobot.modules.sql.global_bans_sql import is_user_gbanned
from VishnuRobot.modules.sql.users_sql import get_user_num_chats
...
def info(update: Update, context: CallbackContext):
    ...
    if user_id not in [bot.id, 777000, 1873246625]:
            ...
            message.reply_photo(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "ʜᴇᴀʟᴛʜ", url="https://t.me/VishnuAssociation/7"
                            ),
                            InlineKeyboardButton(
                                "ᴅɪꜱᴀꜱᴛᴇʀ", url="https://t.me/VishnuAssociation/8"
                            ),
                        ],
                    ]
                ),
...
def set_about_me(update: Update, context: CallbackContext):
    ...
    if user_id in [777000, 1873246625]:
    ...
    if message.reply_to_message:
        ...
        if repl_user_id in [bot.id, 777000, 1873246625] and (user_id in DEV_USERS):
...
def set_about_bio(update: Update, context: CallbackContext):
    ...
    if message.reply_to_message:
        ...
        if user_id in [777000, 1873246625] and sender_id not in DEV_USERS:
